<template>
    <table style="border-spacing: 1rem; text-align: center; width: 50rem;" v-if="carts.length !== 0">
        ...
        <tr v-for="item in carts" :key="item.idProduk">
            ...
            <td>
                <button @click="subtractCarts(item.idProduk)" style="width: 5rem;border-radius: 20px;">Remove</button>
            </td>
        </tr>
        <tr>
            <td>
                <button @click="checkout" style="width: 5rem; border-radius: 20px;">Checkout</button>
            </td>
        </tr>
    </table>
</template>
